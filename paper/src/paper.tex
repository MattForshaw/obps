...
partially observable rewards\cite{bubnow}. At every round, an action (represented by an arm
...
reorder it at will. We choose on a search space of 10 reordering policies.
...
+/- 20\% multiplicative noise on the waiting time estimated by the simulator. The overall cost
...
modeling~\cite{feitbook}. A recent approach~\cite{feitresampling} suggests to
...
We publish a declarative workflow that runs simulations and generates all the
figures from this paper. Any question regarding replication can be freely
directed to the authors via e-mail.
...
software packaging approach\ref{nix}. This allows to replicate the experiments on
...
random policy. Indeed, shorter periods lead to better performance. This is perhaps
due to the use of queuing in the scheduling process:
...
the Bandit Feedback policy with $\Delta$ = 1 week.  Observe that here the choice of policy is much less stable. The convergence is
