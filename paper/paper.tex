...
where the more recent history has to be privileged. As explained in Section
6.1, in our experiments $\lambda$ is set to 1 as the distributions considered
are stationary but the reader has to bear in mind that our framework can be use
on different distributions by resorting to different values of $\lambda$ that can
...
\begin{figure*}[]
  ...
  \caption{Illustration of the bandit-based strategy. Every week,
  the wokload from the past week is used to run simulations using every
  reordering policy. The "model" $w$ is updated according
...
be independent from one week to an other, as illustrated in~\cite{jsspp17},
it is reasonable
to assume that the system is in a stationary state. Indeed,~\cite{jsspp17} performs
train/test experiments using the same policy space as we use here. We do provide guidelines as how to incorporate tracking
into our algorithms in Section~\ref{sec:online} by introducing a decaying
discount factor $\lambda$. In our experiments, we use stationary resampled data,
and therefore no decay is necessary: $\lambda$ here is set to $1$.
